...
- **Profiles** (`worker/seed.sql`): Eli (admin), Christian, Mom, Dad, Niece (adults), Ezra, Kiara (kids), Downstairs TV (kiosk, read-only). The admin panel (Me tab, Eli only) edits name/emoji/colour/kind, resets PINs, rotates the pairing code, unpairs devices and shows usage.
